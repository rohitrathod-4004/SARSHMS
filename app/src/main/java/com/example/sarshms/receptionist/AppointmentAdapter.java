...
public class AppointmentAdapter extends RecyclerView.Adapter<AppointmentAdapter.ViewHolder> {
    ...
    private void updateOriginalAppointmentStatus(Appointment appointment, String status, int position) {
        db.collectionGroup("Appointments")
                ...
                .addOnFailureListener(e -> Toast.makeText(context, "Query failed: " + e.getMessage(), Toast.LENGTH_SHORT).show());
    }


    private void updateAppointmentStatus(Appointment appointment, String status, int position) {
        db.collectionGroup("Appointments")
                .whereEqualTo("doctor", appointment.getDoctor())
                .whereEqualTo("date", appointment.getDate())
                .whereEqualTo("time", appointment.getTime())
                ...
                .addOnSuccessListener(querySnapshot -> {
                    for (QueryDocumentSnapshot doc : querySnapshot) {
                        doc.getReference().update("status", status)
                                .addOnSuccessListener(aVoid -> {
                                    doc.getReference().delete(); // Optional: if you still want to delete the record
